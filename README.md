...
| BME/BMP280 | VIN | D13 / GPIO 12 или **3V3** (см. ниже) |
| BME/BMP280 | GND | GND |
...
показывается. Если VIN сидит на D13, после опроса GPIO 12 снимает питание
модуля на время Deep Sleep.

На многих модулях BME/BMP280 питание с **D13 / GPIO 12 не хватает**: пин ESP
даёт мало тока, GPIO 12 ещё и strapping с внутренней подтяжкой к земле. В
Serial тогда бывает `PWR=1 SDA=0 SCL=0` и `I2C … no ACK (5)` — шина зажата,
карточка датчика пустая. В таком случае перенесите **VIN на вывод 3V3**
FireBeetle (GND, SDA и SCL не трогайте) и нажмите Reset. Стабилизатор платы
держит 3,3 В нормально; ток датчика маленький, для Deep Sleep это приемлемо.
После успешного подключения в логе должны быть `SDA=1 SCL=1`, `I2C scan: 0x76`
(или `0x77`) и строка `Sensor data:`.
...
![Форма входа E-Ink Control Desk](docs/images/control-desk-login.png)
...
### 2. Задайте место, экран и плату

В разделе **«01 — Экран»** настройте:
...
6. Тема, размер шрифта, скругление, зазор и рамки — по вкусу. Для этой платы
   не включайте цветные режимы панели: экран чёрно-белый.

В разделе **«02 — Плата»**:

1. Интервал обновления: пресеты от 1 минуты до 1 дня или **«Свой интервал»**
   (целое число от 1 до 1440 минут).
2. При желании **«Ночной интервал»** — более редкие пробуждения в выбранном
   окне по часовому поясу города.
3. **«Кешировать PNG»** — если погода не изменилась, плата получит 304 и не
   будет зря Fully Refresh.
4. Разрешение **7.5″ 800×480 · ландшафт** и цвета **2 цвета · чёрный / белый**.
...
- перетаскивайте карточку по превью, чтобы изменить место;
...
В разделе **«03 — Ссылка устройства»** нажмите **«Копировать URL»**. Нужен
...
2. Выполняет HTTPS-запрос к `DEVICE_BASE_URL/screen.png` и передаёт показания
   ...
   рисует датчик в карточке и процент заряда в шапке (если шапка это показывает).
...
| `SDA=0 SCL=0` / `no ACK (5)` | VIN с D13 не питает модуль: перенесите VIN на **3V3** |
| `I2C scan: none` / `no ACK (2)` | Шина свободна, чипа нет: SDA/SCL, общий GND |
...
| Файл | Содержание |
|---|---|
| [docs/GETTING-STARTED.md](docs/GETTING-STARTED.md) | Короткий старт |
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | Цикл wake → PNG → sleep |
| [docs/CONFIGURATION.md](docs/CONFIGURATION.md) | `secrets.h` и PlatformIO |
| [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) | Сборка и контракт с сайтом |
| [docs/TESTING.md](docs/TESTING.md) | Проверки по Serial и экрану |
