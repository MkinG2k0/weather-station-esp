...
`0x77`. Если подключён BME280, считываются температура, давление и влажность;
для BMP280 — температура и давление. Результат выводится в Serial Monitor.
...
качество воздуха, облачность, восход и закат, УФ-индекс и другие показатели.
Макет ограничен **8 карточками и двумя рядами**. Если комбинация не помещается,
редактор не применит её и покажет сообщение.

Предпросмотр погоды обновляется автоматически. Ссылка **«Открыть PNG»**
показывает именно то изображение 800×480, которое скачает ESP32.
...
1. ESP32 просыпается и подключается к Wi-Fi.
2. Выполняет HTTPS-запрос к `DEVICE_BASE_URL/screen.png`.
